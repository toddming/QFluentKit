...
QColor Indicator::backgroundColor() const {
    bool isDark = sTheme->isDarkMode();

    if (isChecked()) {
        QColor color = sTheme->themeColor();
        if (!isEnabled()) {
            return isDark ? QColor(255, 255, 255, 41) : QColor(0, 0, 0, 56);
        }
        if (property("isPressed").toBool()) {
            return sTheme->themeColor(Theme::ThemeColorType::LIGHT_2);
        }
...
void SwitchButton::initWidget() {
    ...
    sTheme->registerWidget(this, Theme::StyleSheetType::SWITCH_BUTTON);
