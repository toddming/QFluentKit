    ...
    case QStyle::CC_ScrollBar:
    {
        if (const QStyleOptionSlider* sopt = qstyleoption_cast<const QStyleOptionSlider*>(option))
        {
            ...
            if (m_isExpand)
            {
                ...
                painter->setPen(Qt::NoPen);
                painter->setBrush(sTheme->isDarkMode() ? QColor(44, 44, 44, 245) : QColor(252, 252, 252, 217)); // 滑槽
                painter->drawRoundedRect(scrollBarRect, 6, 6);
                //指示器绘制 center()在此处不适用 存在外围边距宽度 需手动计算
                int sideLength = 8;
                painter->setBrush(sTheme->isDarkMode() ? QColor(255, 255, 255, 139) : QColor(0, 0, 0, 114));

                QMap<QString, QString> attrs;
                attrs["fill"] = sTheme->isDarkMode() ? "#FFFFFF" : "#000000";
                attrs["fill-opacity"] = sTheme->isDarkMode() ?  "0.54" : "0.45";
                ...
                if (sopt->orientation == Qt::Horizontal)
                {
                    ...
                    // 左三角
                    Icon::drawSvgIcon(painter, Icon::IconType::CARE_LEFT_SOLID, leftIndicatorRect, attrs);

                    // 右三角
                    Icon::drawSvgIcon(painter, Icon::IconType::CARE_RIGHT_SOLID, rightIndicatorRect, attrs);
                } else {
                    ...
                    // 上三角
                    Icon::drawSvgIcon(painter, Icon::IconType::CARE_UP_SOLID, upIndicatorRect, attrs);

                    // 下三角
                    Icon::drawSvgIcon(painter, Icon::IconType::CARE_DOWN_SOLID, downIndicatorRect, attrs);
