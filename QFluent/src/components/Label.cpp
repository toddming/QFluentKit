...
FluentLabelBase::FluentLabelBase(int fontSize, QFont::Weight weight, QWidget* parent)
    : QLabel(parent) {
    setFont(sTheme->getFont(fontSize, weight));
    init();
}
...
void FluentLabelBase::init() {
    sTheme->registerWidget(this, Theme::StyleSheetType::LABEL);

    setTextColor(sTheme->isDarkMode() ? Qt::white : Qt::black);

    connect(sTheme, &Theme::themeTypeChanged, this, [=](Theme::ThemeType theme){
        setTextColor(theme == Theme::ThemeType::DARK ? Qt::white : Qt::black);
    });
}
