...
NavigationBarPushButton::NavigationBarPushButton(Icon::IconType icon, const QString& text, bool isSelectable,
                                              Icon::IconType selectedIcon, QWidget* parent)
    ...
    setIndicatorColor(sTheme->themeColor(), sTheme->themeColor());
    sTheme->setFont(this, 12);
}
...
void NavigationBarPushButton::setSelectedIcon(Icon::IconType icon) {
    _selectedIcon = icon;
    update();
}
...
void NavigationBarPushButton::_drawBackground(QPainter& painter) {
    if (isSelected) {
        QColor bg = sTheme->isDarkMode() ? QColor(255, 255, 255, 42) : Qt::white;
        painter.setBrush(bg);
        painter.drawRoundedRect(rect(), 5, 5);
        // draw indicator
        QColor indicatorColor = sTheme->themeColor();
        if (!isPressed) {
...
void NavigationBarPushButton::_drawIcon(QPainter& painter) {
    ...
    Icon::IconType selectedIcon = _selectedIcon != Icon::IconType::NONE ? _selectedIcon : m_icon;
    if (isSelected) {
        if (selectedIcon != Icon::IconType::NONE) {
            QMap<QString, QString> attrs;
            attrs["fill"] = sTheme->themeColor().name();
...
void NavigationBarPushButton::_drawText(QPainter& painter) {
    ...
    QColor textColor = isSelected ? sTheme->themeColor() : (sTheme->isDarkMode() ? Qt::white : Qt::black);
...
void NavigationBar::addItem(const QString& routeKey, Icon::IconType icon, const QString& text,
...
void NavigationBar::insertItem(int index, const QString& routeKey, Icon::IconType icon, const QString& text,
                             const std::function<void()>& onClick, bool selectable,
                             Icon::IconType selectedIcon,
                             NavigationItemPosition position) {
    if (m_items.contains(routeKey)) {
