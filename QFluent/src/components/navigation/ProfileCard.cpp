    ...
    : QWidget(parent)
{
    ...
    // 设置 emailLabel 颜色
    QColor emailColor = sTheme->isDarkMode() ? QColor(206, 206, 206) : QColor(96, 96, 96);
    emailLabel->setStyleSheet(QStringLiteral("QLabel{color: %1}").arg(emailColor.name()));

    // 设置 nameLabel 颜色
    QColor nameColor = sTheme->isDarkMode() ? QColor(255, 255, 255) : QColor(0, 0, 0);
    nameLabel->setStyleSheet(QStringLiteral("QLabel{color: %1}").arg(nameColor.name()));

    // 设置按钮字体
    sTheme->setFont(logoutButton, 13);
