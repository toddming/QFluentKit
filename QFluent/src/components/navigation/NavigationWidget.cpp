...
QColor NavigationWidget::textColor() {
    return sTheme->isDarkMode() ? darkTextColor : lightTextColor;
}
...
NavigationPushButton::NavigationPushButton(Icon::IconType icon, const QString& text,
...
void NavigationPushButton::setIcon(Icon::IconType icon) {
    m_icon = icon;
    update();
}
...
void NavigationPushButton::paintEvent(QPaintEvent* e) {
    ...
    int left = (m_icon == Icon::IconType::NONE) ? pl + 16 : 44 + pl;
    painter.drawText(QRectF(left, 0, width() - 13 - left - pr, height()), Qt::AlignVCenter, text());
}

// NavigationToolButton 实现
NavigationToolButton::NavigationToolButton(Icon::IconType icon, QWidget* parent)
...
NavigationTreeItem::NavigationTreeItem(Icon::IconType icon, const QString& text,
...
void NavigationTreeItem::paintEvent(QPaintEvent* e) {
    ...
    Icon::drawSvgIcon(&painter, Icon::IconType::ARROW_DOWN, QRectF(-5, -5, 9.6, 9.6));
}
...
NavigationTreeWidget::NavigationTreeWidget(Icon::IconType icon, const QString& text,
                                         bool isSelectable, QWidget* parent)
...
void NavigationTreeWidget::setIcon(Icon::IconType icon) {
