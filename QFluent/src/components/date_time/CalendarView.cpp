...
void ScrollButton::paintEvent(QPaintEvent* event)
{
    ...
    attrs["fill"] = sTheme->isDarkMode() ? "#5e5e5e" : "#9c9c9c";
...
void ScrollItemDelegate::_drawBackground(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    if (index != selectedIndex) {
        ...
        painter->setPen(sTheme->themeColor());
    }

    if (index == currentIndex) {
        if (index == pressedIndex) {
            painter->setBrush(sTheme->themeColor(Theme::ThemeColorType::LIGHT_2));
        } else if (option.state & QStyle::State_MouseOver) {
            painter->setBrush(sTheme->themeColor(Theme::ThemeColorType::LIGHT_1));
        } else {
...
void ScrollItemDelegate::_drawText(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    if (index == currentIndex) {
        painter->setPen(sTheme->isDarkMode() ? Qt::black : Qt::white);
    } else {
        painter->setPen(sTheme->isDarkMode() ? Qt::white : Qt::black);
...
void CalendarViewBase::initWidget() {
    ...
    sTheme->registerWidget(this, Theme::StyleSheetType::CALENDAR_PICKER);
