...
void TableItemDelegate::initStyleOption(QStyleOptionViewItem* option, const QModelIndex& index) const {
    ...
    // option->font = index.data(Qt::FontRole).value<QFont>() || sTheme->getFont(13);
    // option->font = sTheme->getFont(13);

    QColor textColor = sTheme->isDarkMode() ? Qt::white : Qt::black;
...
void TableItemDelegate::_drawCheckBox(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    if (checkState == Qt::Unchecked) {
        ...
        painter->setPen(sTheme->themeColor()); // 未实现
        painter->setBrush(sTheme->themeColor());
        painter->drawRoundedRect(rect, r, r);
